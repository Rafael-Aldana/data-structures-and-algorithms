...
const count = (target, input) => {
  ...
  for (let arr of input) {
    result += arr.reduce((acumulator, currentValue) => currentValue === target ? acumulator + 1 : acumulator, 0);
  } return result;
};
...
const totalSum = (input) => {
  let result = 0;
  for (let arr of input) {
    result += arr.reduce((acumulator, currentValue) => acumulator + currentValue, 0);
  } return result;
};
...
let findMaleAndFemale = (data) => {
  let result = data.filter(character => character.gender === 'male' || character.gender === 'female').map(character => character.name);
